...
describe("FreshRSS images helpers", function()
    ...
    it("uses hidden placeholder when images are off", function()
        local html = [[<img src="https://a/1.png">]]
        local out = Images.rewriteHtml(html, {}, { show_images = false })
        assert.truthy(out:find("%[image hidden%]"))
        assert.equals("[image hidden]", Images.placeholderText({ show_images = false }))
        assert.equals("[image · tap to fetch]", Images.placeholderText({ show_images = true }))
    end)

    it("counts cached vs total images", function()
        local data_dir = os.tmpname()
        os.remove(data_dir)
        local imgdir = Images.ensureDirectory(Images.directory(data_dir))
        local name = Images.filenameForUrl("https://cdn/one.png")
        local f = assert(io.open(imgdir .. "/" .. name, "wb"))
        f:write("\x89PNG\r\n\x1a\nxxxx")
        f:close()
        local html = [[<img src="https://cdn/one.png"><img src="https://cdn/two.png">]]
        local stats = Images.countImageStats(html, data_dir)
        assert.equals(2, stats.total)
        assert.equals(1, stats.cached)
